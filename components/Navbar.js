...
export default function Navbar() {
  ...
  return (
    <nav className='h-24 w-full  text-primary flex justify-between items-center  p-10 xl:px-40  overflow-hidden'>
        ...
        <div className={`flex justify-center fixed overflow-hidden  top-0 bottom-0  left-0 duration-500  ${visibility.value? "right-0 ":"right-full"} bg-primary z-10 box-border`}>
          <X onClick={close} className='lg:hidden cursor-pointer text-5xl block text-white absolute right-4 top-4'/>
          <ul className=' w-full gap-4 flex flex-col text-sm justify-center whitespace-nowrap'>
